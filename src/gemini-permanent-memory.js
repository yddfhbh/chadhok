...
const PERMANENT_MEMORY_TRIGGER_PATTERN = /(기억해줘|기억해둬|기억해)/g;
const PERMANENT_MEMORY_USAGE_PATTERN = /\[\[\s*PERMANENT_MEMORY_USED\s*:\s*([^\]]*)\]\]/gi;
const PERMANENT_MEMORY_INFERENCE_MIN_SCORE = 9;
...
export function extractPercentPermanentMemory(content) {
  ...
  return cleanPermanentMemoryText(body.replace(PERMANENT_MEMORY_TRIGGER_PATTERN, " "));
}
...
export function extractPermanentMemoryUsage(answer, allowedIds = []) {
  ...
  const usedIds = [];
  const cleanText = String(answer ?? "")
    .replace(PERMANENT_MEMORY_USAGE_PATTERN, (_, rawIds) => {
  ...
  return entries
    .filter((entry) => (
      scoreSearchMatch(answerTerms, createSearchTerms(entry.text))
        >= PERMANENT_MEMORY_INFERENCE_MIN_SCORE
    ))
    .map((entry) => entry.id);
}
  ...
  async add({ scopeId, text, authorId, authorName }) {
    ...
    if (normalizedText.length > permanentMemoryMaxTextLength) {
      throw new RangeError(
        `Permanent memory text exceeds ${permanentMemoryMaxTextLength} characters.`
      );
    }
